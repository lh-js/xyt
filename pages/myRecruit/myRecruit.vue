<template>
	<view class="forum-content">
		<view class="forum-box" v-for="(item,index) in myRecruitList" :key="item._id">
			<view class="forum-item" @click="toRecruitDetail(item._id)">
				<view class="forum-item-title">
					{{item.recruitName}}
				</view>
				<image v-if="item.recruitImg!=''" :src="item.recruitImg" mode="aspectFill"></image>
			</view>
			<view class="forum-state">
				...
				<view class="">
					{{item.recruitState == 0 ? '未认领' : '已认领'}}
				</view>
				<view style="display: flex;align-items: center;">
					<view style="display: flex;flex-direction: column;">
						是否匿名
						<switch style="margin-right: 3px;" :checked="item.isRecruitAnonymous"
							@change="switchChange($event,item._id)" type="switch" />
					</view>
					<button type="warn" size="mini" @click="removeRecruit(item._id)">删除</button>
				</view>
			</view>
			<uni-collapse style="width: 100%;">
				<uni-collapse-item title="认领列表" :open="false">
					<view class="recruit-user-item">
						<!-- <uni-collapse accordion v-model="value"> -->
						<view v-for="(item1,index1) in item.recruit_info" :key="item1._id"
							...
							<view class="recruit-user">
								...
								<view class="dorecruit-time">
									{{item1.doRecruitState==0 ? '' : '已认领'}}
									<uni-dateformat :date="item1.doRecruitTime"></uni-dateformat>
								</view>
							</view>
							<!-- 				</template> -->
								...
								<view class="recruit-user-button">
									<view>
										<button type="primary" size="mini"
											:disabled="item1.doRecruitState==1 || item.recruitState==1"
											@click="doRecruit(item._id,item1._id)">{{item1.doRecruitState==0 && item.recruitState==0 ? '确认认领' : '已认领'}}</button>
									</view>
								</view>
							</view>
							<!-- </uni-collapse-item> -->
						...
						<view style="width: 100%;display: flex;justify-content: center;color: #999999;"
							v-if="item.recruit_info.length==0">
							此物品暂时无人认领
						</view>
						<!-- </uni-collapse> -->
					</view>
...
<script>
	export default {
		...
		methods: {
					...
					success: (res1) => {
						...
						uniCloud.callFunction({
							...
							.then((res) => {
								uni.hideLoading()
								console.log(res)
								this.myRecruitList = res.result.data
								if (this.myRecruitList == "") {
			...
			doRecruit(recruitId, userId) { //认领物品
				var that = this
				uni.showModal({
					title: '提示',
					content: '确认是他认领的物品吗？',
